...
function normalizeOptions(options, isNet6OrHigher) {
    ...
    const result = Object.assign(Object.assign({}, flags), { diagnostics: Array.isArray(diagnostics)
            ? diagnostics.join(' ')
            : diagnostics, include: Array.isArray(include) ? include.join(' ') : include, exclude: Array.isArray(exclude) ? exclude.join(' ') : exclude });
    // Specifying --verify-no-changes false does not work, so we only add the switch when we want to run the check only
    if (!fix && check) {
        if (isNet6OrHigher) {
            return Object.assign(Object.assign({}, result), { verifyNoChanges: true });
        }
        return Object.assign(Object.assign({}, result), { check: true }); // The --check flag is for .NET 5 and older
    }
    return result;
}
